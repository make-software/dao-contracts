    ...
    match constructor_name.as_str() {
        "init" => {
            let odra_address = odra::types::Address::try_from(contract_package_hash)
                ...
                .unwrap_or_revert();
            let mut contract_ref = dao::core_contracts::KycNftContractRef::at(&odra_address);
...
fn init() {
    ...
    let (mut _contract, _): (dao::core_contracts::KycNftContract, _) =
...
fn is_whitelisted() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::core_contracts::KycNftContract, _) =
        odra::StaticInstance::instance(&[
            "",
            "",
            "",
