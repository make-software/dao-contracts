...
fn call() {
    let schemas = vec![
        odra::casper::utils::build_event(
            String::from("AdminVotingCreated"),
            vec![
                (
                    "contract_to_update",
    ...
    match constructor_name.as_str() {
        "init" => {
            let odra_address = odra::types::Address::try_from(contract_package_hash)
                ...
                .unwrap_or_revert();
            let mut contract_ref = dao::voting_contracts::AdminContractRef::at(&odra_address);
...
fn init() {
    ...
    let (mut _contract, _): (dao::voting_contracts::AdminContract, _) =
...
fn create_voting() {
    ...
    let contract_to_update =
        odra::casper::casper_contract::contract_api::runtime::get_named_arg("contract_to_update");
    ...
    _contract.create_voting(contract_to_update, action, address, stake);
}
#[no_mangle]
fn vote() {
    odra::casper::utils::assert_no_attached_value();
...
fn voting_exists() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::voting_contracts::AdminContract, _) =
        odra::StaticInstance::instance(&[
            "",
            "",
            "",
