...
pub fn setup_admin() -> (AdminContractTest, ReputationContractTest) {
    ...
    let (variable_repo_contract, mut reputation_token_contract, va_token_contract) =
        setup_base_contracts(informal_quorum, formal_quorum, total_onboarded);
...
pub fn setup_repo_voter(
    ...
    let (mut variable_repo_contract, mut reputation_token_contract, va_token_contract) =
        setup_base_contracts(informal_quorum, formal_quorum, total_onboarded);
...
fn setup_base_contracts(
    ...
    let va_token_contract = setup_va_token_contract(&env, total_onboarded);
...
pub fn setup_voting_contract_with_informal_voting(
    ...
    let (mut mock_voter_contract, _variable_repository_contract, reputation_token_contract, _) =
...
pub fn setup_va_token_contract(env: &TestEnv, total_onboarded: usize) -> DaoOwnedNftContractTest {
    let mut va_token_contract = DaoOwnedNftContractTest::new(
        env,
        "va_token_name".to_string(),
        "va_token_symbol".to_string(),
        "va_base_uri".to_string(),
    );

    for i in 0..total_onboarded {
        va_token_contract
            .mint(env.get_account(i), TokenId::from(i))
            .unwrap();
    }
