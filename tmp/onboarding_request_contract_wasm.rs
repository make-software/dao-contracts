...
fn call() {
    let schemas = vec![
        odra::casper::utils::build_event(
            String::from("OnboardingVotingCreated"),
            vec![
                ("reason", odra::casper::casper_types::CLType::String),
                ("cspr_deposit", odra::casper::casper_types::CLType::U512),
    ...
    match constructor_name.as_str() {
        "init" => {
            ...
            let mut contract_ref =
                dao::voting_contracts::OnboardingRequestContractRef::at(&odra_address);
...
fn init() {
    ...
    let (mut _contract, _): (dao::voting_contracts::OnboardingRequestContract, _) =
...
fn create_voting() {
    ...
    let (mut _contract, _): (dao::voting_contracts::OnboardingRequestContract, _) =
        odra::StaticInstance::instance(&[
            ...
            "refs#va_token",
            "refs#kyc_token",
            "voting#voting_states",
            "voting#ballots",
            "voting#voters",
            "voting#configurations",
            "voting#active_votings",
        ]);
    let reason = odra::casper::casper_contract::contract_api::runtime::get_named_arg("reason");
    _contract.create_voting(reason);
    odra::casper::utils::clear_attached_value();
}
#[no_mangle]
fn get_cspr_balance() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::voting_contracts::OnboardingRequestContract, _) =
        odra::StaticInstance::instance(&[
            "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
            "", "", "", "", "", "", "", "", "",
        ]);
