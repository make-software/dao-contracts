...
fn call() {
    let schemas = vec![
        odra::casper::utils::build_event(
            String::from("KycVotingCreated"),
            vec![
                ("subject_address", odra::casper::casper_types::CLType::Key),
    ...
    match constructor_name.as_str() {
        "init" => {
            let odra_address = odra::types::Address::try_from(contract_package_hash)
                ...
                .unwrap_or_revert();
            let mut contract_ref = dao::voting_contracts::KycVoterContractRef::at(&odra_address);
            ...
            contract_ref.init(variable_repository, reputation_token, va_token, kyc_token);
        }
...
fn init() {
    ...
    let (mut _contract, _): (dao::voting_contracts::KycVoterContract, _) =
    ...
    let kyc_token =
        odra::casper::casper_contract::contract_api::runtime::get_named_arg("kyc_token");
    _contract.init(variable_repository, reputation_token, va_token, kyc_token);
}
#[no_mangle]
fn create_voting() {
    odra::casper::utils::assert_no_attached_value();
    ...
    let subject_address =
        odra::casper::casper_contract::contract_api::runtime::get_named_arg("subject_address");
    let document_hash =
    ...
    _contract.create_voting(subject_address, document_hash, stake);
}
#[no_mangle]
fn vote() {
    odra::casper::utils::assert_no_attached_value();
...
fn voting_exists() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::voting_contracts::KycVoterContract, _) =
        odra::StaticInstance::instance(&[
            "",
            "",
            "",
