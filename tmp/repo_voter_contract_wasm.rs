    ...
    match constructor_name.as_str() {
        "init" => {
            let odra_address = odra::types::Address::try_from(contract_package_hash)
                ...
                .unwrap_or_revert();
            let mut contract_ref = dao::voting_contracts::RepoVoterContractRef::at(&odra_address);
...
fn init() {
    ...
    let (mut _contract, _): (dao::voting_contracts::RepoVoterContract, _) =
...
fn create_voting() {
    ...
    let variable_repo_to_edit = odra::casper::casper_contract::contract_api::runtime::get_named_arg(
        "variable_repo_to_edit",
    );
    ...
    _contract.create_voting(variable_repo_to_edit, key, value, activation_time, stake);
}
#[no_mangle]
fn vote() {
    odra::casper::utils::assert_no_attached_value();
...
fn voting_exists() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::voting_contracts::RepoVoterContract, _) =
        odra::StaticInstance::instance(&[
            "",
            "",
            "",
