...
fn call() {
    let schemas = vec![
        odra::casper::utils::build_event(
            String::from("ReputationVotingCreated"),
            vec![
                ("account", odra::casper::casper_types::CLType::Key),
                ("action", odra::casper::casper_types::CLType::U32),
    ...
    match constructor_name.as_str() {
        "init" => {
            ...
            let mut contract_ref =
                dao::voting_contracts::ReputationVoterContractRef::at(&odra_address);
...
fn init() {
    ...
    let (mut _contract, _): (dao::voting_contracts::ReputationVoterContract, _) =
...
fn create_voting() {
    ...
    let account = odra::casper::casper_contract::contract_api::runtime::get_named_arg("account");
    let action = odra::casper::casper_contract::contract_api::runtime::get_named_arg("action");
    ...
    _contract.create_voting(account, action, amount, document_hash, stake);
}
#[no_mangle]
fn vote() {
    odra::casper::utils::assert_no_attached_value();
...
fn voting_exists() {
    odra::casper::utils::assert_no_attached_value();
    let (_contract, _): (dao::voting_contracts::ReputationVoterContract, _) =
        odra::StaticInstance::instance(&[
            "",
            "",
            "",
