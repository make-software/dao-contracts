...
use casper_dao_utils::{Error, TestEnv, Address, consts};
use casper_types::{bytesrepr::{ToBytes, FromBytes, Bytes}, U256, RuntimeArgs};
...
fn test_create_voting() {
    // Create voting
    let (env, mut repo_voter_contract, variable_repo_contract, _reputation_token_contract) = create_voting();
    
    // check voting event
    repo_voter_contract.assert_event_at(1, VotingCreated {
...
#[test]
fn test_finishing_vote_before_end() {
    // create voting
    let (_env, mut repo_voter_contract, _variable_repo_contract, _reputation_token_contract) = create_voting();
...
fn test_informal_vote_without_a_quorum() {
    // create voting
    let (env, mut repo_voter_contract, _variable_repo_contract, _reputation_token_contract) = create_voting();
...
#[test]
fn test_informal_vote_rejected() {
    // create voting
    let (env, mut repo_voter_contract, _variable_repo_contract, _reputation_token_contract) = create_voting();
    let voting_id = VotingId::from(0);
    ...
    // cast votes against
    repo_voter_contract.as_account(env.get_account(1)).vote(voting_id, false, U256::from(500)).unwrap();
    repo_voter_contract.as_account(env.get_account(1)).vote(voting_id, false, U256::from(500)).unwrap();
    ...
    repo_voter_contract.assert_event_at(5, InformalVotingEnded {
...
#[test]
fn test_informal_vote_converted() {
    // create voting
    let (env, repo_voter_contract, _variable_repo_contract, _reputation_token_contract) = create_formal_voting();
    let voting_id = VotingId::from(0);
    ...
    repo_voter_contract.assert_event_at(6, VotingCreated {
        creator: env.get_account(0),
        voting_id: VotingId::from(1),
        stake: 500.into(),
    });
    ...
    // with initial vote
    repo_voter_contract.assert_event_at(7, VoteCast {
        voter: env.get_account(0),
        voting_id: 1.into(),
        choice: true,
        stake: 500.into(),
...
fn prepare_variable_repo(mut variable_repo_contract: VariableRepositoryContractTest) -> VariableRepositoryContractTest {
    variable_repo_contract.update_at(consts::INFORMAL_VOTING_QUORUM.into(), U256::from(2).to_bytes().unwrap().into(), None).unwrap();
...
fn create_voting() -> (TestEnv, RepoVoterContractTest, VariableRepositoryContractTest, ReputationContractTest) {
    ...
    repo_voter_contract.create_voting(Address::from(variable_repo_contract.get_package_hash()), "variable_name".into(), into_bytes("new_value"), Some(3600), U256::from(500)).unwrap();
    (env, repo_voter_contract, variable_repo_contract, reputation_token_contract)
}

fn create_formal_voting() -> (TestEnv, RepoVoterContractTest, VariableRepositoryContractTest, ReputationContractTest) {
    let (env, mut repo_voter_contract, variable_repo_contract, reputation_token_contract) = create_voting();
    let voting_id = VotingId::from(0);
    let voting: Voting = repo_voter_contract.get_voting(voting_id);

    // cast votes
    repo_voter_contract.as_account(env.get_account(1)).vote(voting_id, true, U256::from(500)).unwrap();
    repo_voter_contract.as_account(env.get_account(1)).vote(voting_id, false, U256::from(500)).unwrap();
