...
pub trait BidEscrowContractInterface {
    ...
    fn submit_result(&mut self, bid_id: BidId, result: Descrition);
}
...
impl BidEscrowContractInterface for BidEscrowContract {
    fn pick_bid(&mut self,worker:Address,description:Descrition,required_stake_for_va:ReputationAmount) {
        if worker == caller() {
