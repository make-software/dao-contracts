...
build-contracts:
	...
	wasm-strip target/wasm32-unknown-unknown/release/erc20_contract.wasm 2>/dev/null | true
...
test: build-contracts prepare-proxy-getter
	...
	cp target/wasm32-unknown-unknown/release/erc20_contract.wasm tests/wasm
	cargo test -p tests $$TEST_NAME
	cargo test -p erc20 $$TEST_NAME --features=test-support --no-default-features
