...
fn test_balance() {
    ...
    bid_escrow.pick_bid_with_cspr_amount(worker, DocumentHash::from(b"some hash".to_vec()), 60, Some(500.into()), 500.into());

    assert_eq!(bid_escrow.get_env().get_address_cspr_balance(bid_escrow_address), 500.into());
    assert_eq!(bid_escrow.get_env().get_address_cspr_balance(job_poster), job_poster_balance - U512::from(500));
}